...
import com.example.demo.dto.FinancialHealthCareResponseDTO;
...
public class AIService {
    ...
    private final UserService userService;
        ...
        public BudgetPlannerResponseDTO smartBudgetPlanner(Long userId, int year, int month) {
            ...
            FinancialNarrativeResponseDTO obj= generateFinancialNarrative(userId, year, month);
            String prompt = """
                    You are a financial planning assistant.

                    User monthly income: ₹%s
                    Total spending for the month: ₹%s
                    Highest spending of the month: ₹%s
                    Lowest spending of the month: ₹%s
                    Monthly expenditure summary: %s
                    ...
                    """
                    .formatted(salary, obj.getTotalSpending(),obj.getHighestSpendingCategory(),obj.getLowestSpendingCategory(),obj.getNarrative());
        ...
        public FinancialHealthCareResponseDTO aiFinancialHealthScore(Long userId, int year, int month) {
            ...
            // 5️⃣ Savings ratio
            BigDecimal savings = salary.subtract(totalSpending);
            BigDecimal savingsRatio = (salary.compareTo(BigDecimal.ZERO) > 0) ?
                    savings.divide(salary, 4, RoundingMode.HALF_UP) : BigDecimal.ZERO;

            // 6️⃣ Risky category count (categories > 40% of total spending)
            int riskyCount = 0;
            for (BigDecimal amt : categoryTotals.values()) {
                BigDecimal percentage = totalSpending.compareTo(BigDecimal.ZERO) > 0 ?
                        amt.divide(totalSpending, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)) :
                        BigDecimal.ZERO;
                if (percentage.compareTo(BigDecimal.valueOf(40)) > 0) {
                    riskyCount++;
                }
            ...
            // 7️⃣ Calculate health score (0-100)
            // Base 50 + 40 * savingsRatio - 5 * riskyCount
            int score = 50 + (int)(40 * savingsRatio.doubleValue()) - (5 * riskyCount);
            score = Math.min(Math.max(score, 0), 100); // clamp between 0 and 100

            // 8️⃣ Map score to health level
            String healthLevel;
            if (score <= 40) healthLevel = "LOW";
            else if (score <= 60) healthLevel = "MODERATE";
            else if (score <= 80) healthLevel = "GOOD";
            else healthLevel = "EXCELLENT";

            // 9️⃣ AI narrative prompt
            String aiPrompt = """
                    ...
                    - Total spending: ₹%s
                    - Savings ratio: %s%%
                    - Number of risky categories: %d
                    - Health score: %d (0-100), Level: %s

                    Generate a motivational, user-friendly explanation of this financial health score:
                    - Highlight strong points
                    - Highlight areas to improve
                    - Keep it concise (4-6 sentences), clear and actionable.
                    """
                    .formatted(salary, totalSpending, savingsRatio.multiply(BigDecimal.valueOf(100)).setScale(2, RoundingMode.HALF_UP), riskyCount, score, healthLevel);

            String explanation = chatClient.prompt()
                    .system("You are a structured financial advisor for users.")
                    .user(aiPrompt)
                    .call()
                    .content();

            // 10️⃣ Build response DTO
            FinancialHealthCareResponseDTO response = new FinancialHealthCareResponseDTO();
            response.setHealthScore(score);
            response.setHealthLevel(healthLevel);
